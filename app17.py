...
# Input for student name
    ...
    # Validation to ensure Hubspot link is not empty
    if not hubspot_deal.strip():
        st.error("Hubspot Deal link cannot be empty. Please provide the link before proceeding.")
        st.stop()
...
# Replace the current FAO credentials and admission sections with:
    ...
    def sort_counselors(df, package_type):
        ...
        if "AC" in package_type and '# AC spots left after recommendations' in df.columns:
            df = df[df['# AC spots left after recommendations'] >=1]
        elif "CB" in package_type and '# CB spots left after recommendations' in df.columns:
            df = df[df['# CB spots left after recommendations'] >=1]
